...
void Renderer::Render(GLFWwindow* window, Camera* camera, Controller* controller) {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, sr.NormalMapQuad[0]);
        normalMapShader.setMat4("model", model);
        normalMapShader.setVec3("viewPos", camera->Position);
        normalMapShader.setVec3("lightPos", sr.getpointLights()[0]);
        renderQuad();
...
unsigned int quadNormVAO = 0;
unsigned int quadNormVBO;
void renderQuad() {
    if (quadNormVAO == 0) {
        // positions
        ...
        // configure plane VAO
        glGenVertexArrays(1, &quadNormVAO);
        glGenBuffers(1, &quadNormVBO);
        glBindVertexArray(quadNormVAO);
        glBindBuffer(GL_ARRAY_BUFFER, quadNormVBO);
